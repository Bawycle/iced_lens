...
1. **Tool-specific handlers (ongoing)**: crop/resize helper methods now live in `state/{crop,resize}.rs`, rotate sidebar/shortcut actions route through `state/tools.rs`, navigation/save/cancel/back logic lives in `state/session.rs`, save/discard flows are handled by `state/persistence.rs`, toolbar/sidebar/canvas/raw-event match arms live in `state/routing.rs`, shared helper methods (`apply_dynamic_transformation`, resize syncing, crop commit hook) sit in `state/helpers.rs`, the public-facing `ViewContext` + `State::view/display_image` glue plus the `State::new`, `EditorTool`, and `Transformation` definitions reside in `component.rs`, and the toolbar/sidebar/canvas/raw-event enums plus parent-facing `Event`/`Message` types live in `messages.rs`. Next up: evaluate whether any remaining helpers (e.g., state struct field docs or preview helpers) should move so `mod.rs` is just the struct definition, re-exports, and tests.
2. **History subsystem**: undo/redo bookkeeping and replay logic now reside in `state/history.rs`; focused unit tests now cover redo truncation, history bounds, and transformation replay, and we can still explore replay-specific error improvements later.
